...
const MonthlyClaimForm = () => {
    ...
    useEffect(() => {
        const fetchClaimStatus = async () => {
            ...
            try {
                const response = await fetch(`https://trf-scholar-1.onrender.com/api/claims/status?email=${storedUser.email}&month=${month}&year=${year}`);
                if (response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://trf-scholar-1.onrender.com/api/claims', {
                method: 'POST',
